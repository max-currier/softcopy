...
def get_recs(matched_isbns, num_recs):
    ...
    st.header('**Step 4: Recommend books based on the ones on your bookshelf.**')
...
def match_title(final_ocr, num_recs, confidence):
    ...
    st.header('**Step 3: Match OCR results to books in our database**')
...
def run_ocr(new_img, num_recs, confidence, verbose):
    ...
    st.header('**Step 2: Read text from your image.**')
...
def main():
    ...
    if choice == 'Home':
        ...
        # image uploader
        st.header('**Step 1: Upload an image of your books.**')
...
def about():
    ...
    st.markdown(
                ...
                1. Scan your image to detect text
                2. Match detected text with books in our database
                3. Make recommendations based off of book matches
    	''')

    # select which phase to view details
    about_options = st.select_slider(
        label='Select a phase to learn more about how it works',
        options=['Image Scan', 'Text Matching', 'Recommendation'], )

    # OCR phase details
    if about_options == 'Image Scan':

        st.subheader('Scanning text with optical character recognition (OCR)')
    ...
    if about_options == 'Text Matching':
        ...
        1. Title and author strings combined and tokenized into list of unigrams

        2. Lists sorted and converted into a set to remove duplicates

        3. Sets converted back into strings and compared to and assigned confidence scores
        corresponding to all title/author combinations in dataset.
    ...
    if about_options == 'Recommendation':
        ...
        * 12,854 unique reviewers
        ...
        to generate 100 abstract features for each book in the Book Crossing data
        ...
        st.markdown('''
        The recommendation engine then uses these 100 properties to **compare the cosine similarity** each of your
